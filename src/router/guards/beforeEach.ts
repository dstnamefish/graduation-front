...
async function handleRouteGuard(
  ...
  // 核心优化：白名单直通车！
  // 如果是前往静态白名单页面（如 Login, 403, 500），无条件直接放行，打破死循环！
  // 注意：Exception404 不在白名单中，这样未登录访问不存在路径也会跳转登录页
  if (isStaticRoute(to.path) || ['Login', 'Exception500', 'Exception403'].includes(to.name as string)) {
    next();
    return;
  }
...
function isStaticRoute(path: string): boolean {
  const checkRoute = (routes: any[], targetPath: string, parentPath = ''): boolean => {
    return routes.some((route) => {
      ...
      // 关键修复：排除 404 通配符路由和带参数的动态路由
      // 否则所有的路径都会被识别为静态路由，导致权限校验失效
      if (route.name === 'Exception404' || route.path.includes('*') || route.path.includes(':')) {
        return false;
      }
