...
<template>
  <ElButton v-bind="$attrs" :type="resolvedType" :loading="props.loading" :class="buttonClasses">
    <div class="flex-cc gap-2">
      <WnSvgIcon v-if="props.loading" icon="solar:round-alt-arrow-right-bold-duotone" :size="props.iconSize" />
      <WnSvgIcon v-else-if="shouldShowIcon" :icon="resolvedIcon" :size="props.iconSize" :class="iconColorClass" />
      <span v-if="shouldShowLabel"><slot>{{ resolvedLabel }}</slot></span>
    </div>
  </ElButton>
</template>
